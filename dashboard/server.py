"""
dashboard/server.py — SONIC Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "SONIC Uploads",
        Path.home() / "Documents" / "SONIC Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'SONIC-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"SONIC Dashboard Port {port}"
        prog_rule  = "SONIC Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="sonic_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until SONIC is run as Administrator.")
...
def _ensure_certs() -> bool:
    ...
    key_p = certs / "sonic.key"
    crt_p = certs / "sonic.crt"
    ...
    try:
        ...
        who = x509.Name([
            x509.NameAttribute(NameOID.COMMON_NAME, "SONIC Dashboard"),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "SONIC"),
        ])
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        certs = BASE_DIR / "config" / "certs"
        return (certs / "sonic.key").exists() and (certs / "sonic.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in SONIC to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('sonic_token','{tok}');
  sessionStorage.setItem('sonic_key','{key}');
  localStorage.setItem('sonic_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to SONIC…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "sonic.key"
