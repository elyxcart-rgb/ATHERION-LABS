"""
Drop-in SONIC plugin template.

Copy this file, rename it (no leading underscore), fill in PLUGIN and run().
No other file needs to change — SONIC discovers this automatically at startup.
"""
...
def run(parameters: dict, player=None, session_memory=None) -> str:
    ...
    player: the SonicUI instance — use player.write_log(f"SONIC: ...") to log,
    ...
    if player:
        try:
            player.write_log(f"SONIC: {result_text}")
        except Exception:
            pass
    return result_text
