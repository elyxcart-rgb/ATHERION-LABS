...
try:
    ...
    notification.notify(title="S.O.N.I.C Reminder", message=message, timeout=15)
    notified = True
except Exception:
    pass
...
if not notified:
    try:
        ...
        ToastNotifier().show_toast("S.O.N.I.C Reminder", message, duration=15, threaded=False)
        notified = True
    except Exception:
        pass
...
if not notified:
    try:
        import subprocess
        script = 'display notification "{{}}" with title "S.O.N.I.C Reminder"'.format(
            message.replace('"', '')
        )
...
"""
    ...
    script_body = f"""# Auto-generated by S.O.N.I.C reminder — do not edit
...
def _schedule_windows(target_dt: datetime, task_name: str,
    ...
    xml_content = (
        ...
        '  <RegistrationInfo><Description>S.O.N.I.C Reminder</Description></RegistrationInfo>\n'
...
def _schedule_mac(target_dt: datetime, task_name: str,
    ...
    label     = f"com.sonic.reminder.{task_name}"
...
def reminder(
    ...
    task_name  = f"SONICReminder_{target_dt.strftime('%Y%m%d_%H%M%S')}"
