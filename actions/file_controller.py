...
def file_controller(
    ...
    validation = validate_tool_call("file_controller", params)
    if not validation.valid:
        if validation.error_type == "INVALID_ACTION":
            supported = get_supported_actions("file_controller")
            return f"Unsupported action: '{action}'. Available: {', '.join(supported)}"
        return f"Validation error: {validation.message}"

    from reliability.paths import resolve_path as reliable_resolve
    resolved = reliable_resolve(path)
    if resolved:
        path = str(resolved)
    ...
    try:
        if action == "list":
            ...
            from reliability.verify import verify_open
            from actions.open_app import open_app
            target = name or path
            v = verify_open(target)
            if not v.success:
                return v.message
            return open_app(parameters={"app_name": target}, player=player)

        else:
            supported = get_supported_actions("file_controller")
            return f"Unknown action: '{action}'. Available: {', '.join(supported)}"
