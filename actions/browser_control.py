...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".sonic_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".sonic_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using SONIC profile")
                sonic = str(Path.home() / ".sonic_profiles" / "firefox_sonic")
                Path(sonic).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(sonic, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".sonic_profiles" / "safari")
        ...
        # SONIC otomasyon profiline geçilir — buraya bir kez giriş yapılan
        # hesaplar sonraki oturumlarda da açık kalır.
        sonic_profile = str(Path.home() / ".sonic_profiles" / self.browser_name)
        Path(sonic_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with SONIC profile: {sonic_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(sonic_profile, **kwargs)
            self._page = await self._adopt_page()
            print(f"[Browser] ✅ Launched [{label}] with SONIC profile "
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "sonic_screenshot.png")
