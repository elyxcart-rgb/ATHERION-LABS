...
PROJECTS_DIR     = Path.home() / "Desktop" / "SonicProjects"
MAX_FIX_ATTEMPTS = 5
...
def _is_rate_limit(error: Exception) -> bool:
    msg = str(error).lower()
    return "429" in msg or "503" in msg or "quota" in msg or "resource_exhausted" in msg or "unavailable" in msg
...
def _build_project(
    ...
    for attempt in range(3):
        try:
            ...
            if attempt < 2:
                msg = f"API busy, retrying in {5 * (attempt + 1)}s..."
                log(msg)
                import time; time.sleep(5 * (attempt + 1))
            else:
                msg = "Google API is overloaded. Please try again in a minute."
                if speak: speak(msg)
                return msg
    ...
    proj_name    = project_name or plan.get("project_name", "sonic_project")
